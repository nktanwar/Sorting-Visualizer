...
import { callState, initialSz } from '../assets/atoms';
...
const SortVisualizer = ({ height, disabled, initialTime }) => {
    ...
    const [initialSize, setInitialSize] = useRecoilState(initialSz);
    ...
    async function partition(arr, low, high) {
        ...
        // [arr[i + 1], arr[high]] = [arr[high], arr[i + 1]]; // Swap pivot into the right place
        return i + 1;
    }
    ...
    const insertionSort = async () => {
        ...
        for (let i = 1; i < newArray.length; i++) {
            ...
            while (j >= 0 && newArray[j] > key) {
                ...
                swap(j + 1, j, newArray);
                // newArray[j + 1] = newArray[j]; // Shift the element
                setArray([...newArray]);
    ...
    const swap = async (index1, index2, newArray) => {
        ...
        await new Promise(resolve => setTimeout(resolve, 2 * time));
        // await new Promise(resolve => setTimeout(resolve, 1000)); // Added delay for rendering
    ...
    return (
        <div className="flex flex-col items-center">
                ...
                <div className="flex justify-start items-end w-full ">
                    {array.map((value, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                style={{ height: `${barHeight}px`, width: `${barWidth}px`, margin: '0 0px' }} // Margin to separate bars
                                className={barClass}
                            />
                        );
                    })}
            ...
            <div className="flex p-3 justify-center bg-blue-100 shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl rounded-2xl">
